...
writeLetter(var emotion, var cause, var today, Function saveMessage) async {
  ...
  String apiKey2 = "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";
  ...
  String userbirth =
      await pref.getString('userBirth') ?? ''; //  pref.getString("userName")
  DateFormat format = DateFormat('yyyy-MM-dd');
  DateTime UserBirth = format.parse(userbirth);
  int userAge = DateTime.now().year - UserBirth.year;

  var job = pref.getString('userJob'); // pref.getString("userJob")

  String input =
      "나이는 $userAge이고, 직업은 $job이야. 내가 느낀 감정은 [$emotion]이고, 원인은 $cause 때문이야. $today";

  Dio _dio1 = Dio(BaseOptions(headers: {
    'Authorization': 'Bearer $apiKey2',
    'Content-Type': 'application/json',
    'OpenAI-Beta': 'assistants=v1'
  }));

  Dio _dio2 = Dio(BaseOptions(headers: {
    'Authorization': 'Bearer $apiKey2',
    'OpenAI-Beta': 'assistants=v1'
  }));
  ...
  // create runs
  var res3 = await _dio1.post(
      "https://api.openai.com/v1/threads/$thread_id/runs",
      data: {'assistant_id': AssistantID2});
  Map<String, dynamic> RunRes = res3.data;
  var run_id = RunRes["id"];
  print(RunRes);

  // retrieve run
  var res4 = await _dio2
  ...
  // search GPT res
  var res5 =
      await _dio1.get("https://api.openai.com/v1/threads/$thread_id/messages");
